...
function updateNavClass(activeTabId) {
  ...
  liproduk.classList.toggle(
    "active",
    activeTabId === "produk" ||
      activeTabId === "stock_in" ||
      activeTabId === "kategori"
  );
...
function updateKaryawanClass() {
  if (currentPath.includes("karyawan.php")) {
    ...
    currentPath.includes("record.php") ||
    currentPath.includes("recordAdmin.php")
  ) {
...
function showConfirmationDelete(id) {
  ...
  if (currentPath.includes("produk.php")) {
    ...
    text.innerHTML =
      "penghapusan akan berdampak pada table produk,apa anda yakin?";
...
function showConfirmationEdit(id) {
  ...
  if (currentPath.includes("karyawan.php")) {
    continueButton.href =
      "input_component/input_karyawan.php?edit_karyawan=" + id;
  } else {
